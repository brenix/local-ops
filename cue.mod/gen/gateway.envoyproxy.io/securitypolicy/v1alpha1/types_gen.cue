...
#SecurityPolicySpec: {
	...
	authorization?: {
		...
		rules?: [...{
			...
			operation?: {
				...
				// If multiple methods are specified, all specified methods are
				// allowed or denied, based on the action of the rule.
				methods: list.MaxItems(16) & [..."GET" | "HEAD" | "POST" | "PUT" | "DELETE" | "CONNECT" | "OPTIONS" | "TRACE" | "PATCH"] & [_, ...]
			}
			...
			principal: {
				...
				// Headers authorize the request based on user identity extracted
				// from custom headers.
				// If multiple headers are specified, all headers must match for
				// the rule to match.
				headers?: list.MaxItems(256) & [...{
					...
					// If multiple values are specified, the rule will match if any of
					// the values match.
					values: list.MaxItems(256) & [...string] & [_, ...]
				}] & [_, ...]
	...
	// BasicAuth defines the configuration for the HTTP Basic
	// Authentication.
	basicAuth?: {
		// This field specifies the header name to forward a successfully
		// authenticated user to
		// the backend. The header will be added to the request with the
	...
	extAuth?: {
		...
		http?: {
			...
			// that path,
			// or else the authorization request will use the path of the
			// original request.
			//
			// Please note that the original request path will be appended to
			// the path specified here.
			// For example, if the original request path is "/hello", and the
			// path specified here is "/auth",
			// then the path of the authorization request will be
			// "/auth/hello". If the path is not specified,
	...
	jwt?: {
		...
		providers: list.MaxItems(4) & [...{
			...
			// LocalJWKS defines how to get the JSON Web Key Sets (JWKS) from
			// a local source.
			localJWKS?: {
				...
				// ValueRef is a reference to a local ConfigMap that contains the
	...
	oidc?: {
		...
		denyRedirect?: {
			// Defines the headers to match against the request to deny
		...
		// Skips OIDC authentication when the request contains a header
		// that will be extracted by the JWT filter. Unless
		// explicitly stated otherwise in the extractFrom field, this will
		// be the "Authorization: Bearer ..." header.
		//
		// The passThroughAuthHeader option is typically used for
		// non-browser clients that may not be able to handle OIDC
