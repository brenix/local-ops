...
// Spec defines the desired state of HTTPRouteFilter.
#HTTPRouteFilterSpec: {
	// HTTPCredentialInjectionFilter defines the configuration to
	// inject credentials into the request.
	// This is useful when the backend service requires credentials in
	// the request, and the original
	// request does not contain them. The filter can inject
	// credentials into the request before forwarding
	// it to the backend service.
	credentialInjection?: {
		credential: {
			// ValueRef is a reference to the secret containing the
			// credentials to be injected.
			// This is an Opaque secret. The credential should be stored in
			// the key
			// "credential", and the value should be the credential to be
			// injected.
			// For example, for basic authentication, the value should be
		...
		// Header is the name of the header where the credentials are
		// injected.
		// If not specified, the credentials are injected into the
		// Authorization header.
		header?: string

		// Whether to overwrite the value or not if the injected headers
		// already exist.
