...
#EnvoyProxySpec: {
	...
	// DisableLuaValidation disables the Lua script validation for Lua
	// EnvoyExtensionPolicies
	disableLuaValidation?: bool | *false
	...
	provider?: {
		...
		kubernetes?: {
			...
			envoyHpa?: {
				...
				behavior?: {
					...
					scaleDown?: {
						...
						// absolute change of 4 pods in a 15s window.
						...
						// desired
						// metric value under which no updates are made to the desired
						// number of
						// replicas (e.g. 0.01 for 1%). Must be greater than or equal to
						// zero. If not
						// set, the default cluster-wide tolerance is applied (by default
						// 10%).
						//
						// For example, if autoscaling is configured with a memory
						// consumption target of 100Mi,
						// and scale-down and scale-up tolerances of 5% and 1%
						// respectively, scaling will be
			...
			// Envoy Proxy.
			envoyPDB?: {
				// MaxUnavailable specifies the maximum amount of pods (can be
				// expressed as integers or as a percentage) that can be
				// unavailable at all times during voluntary disruptions,
				// such as node drains or updates. This setting ensures that your
				// envoy proxy maintains a certain level of availability
				// and resilience during maintenance operations. Cannot be
				// combined with minAvailable.
				maxUnavailable?: (int | string) & {
					string
				}

				// MinAvailable specifies the minimum amount of pods (can be
				// expressed as integers or as a percentage) that must be
				// available at all times during voluntary disruptions,
				// such as node drains or updates. This setting ensures that your
				// envoy proxy maintains a certain level of availability
				// and resilience during maintenance operations. Cannot be
				// combined with maxUnavailable.
	...
	telemetry?: {
		...
		metrics?: {
			// ClusterStatName defines the value of cluster alt_stat_name,
			// determining how cluster stats are named.
			// For more details, see envoy docs:
			...
			// %BACKEND_REFS%: names of all backends referenced in
			// <NAMESPACE>/<NAME>|<NAMESPACE>/<NAME>|... format
			// Only xDS Clusters created for HTTPRoute and GRPCRoute are
			// currently supported.
			// Default:
