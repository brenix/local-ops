...
#ClientTrafficPolicySpec: {
	...
	clientIPDetection?: {
		...
		xForwardedFor?: {
			...
			// the remote IP address to determine the original client’s IP
			// address.
	...
	headers?: {
		...
		// RequestID configures Envoy's behavior for handling the
		// `X-Request-ID` header.
		// Defaults to `Generate` and builds the `X-Request-ID` for every
		// request and ignores pre-existing values from the edge.
		// (An "edge request" refers to a request from an external client
		// to the Envoy entrypoint.)
