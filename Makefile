SHELL := /usr/bin/env bash
export PATH := $(PWD)/bin:$(PATH)

PLATFORM := $(shell uname -s)
CLUSTERS := $(wildcard clusters/*)
COMPONENTS := $(foreach cluster, $(CLUSTERS), $(wildcard $(cluster)/*))

# Enable secondary expansion
.SECONDEXPANSION:

## Render kustomize for a given cell cluster
$(CLUSTERS): $$(wildcard $$@/*)

## Render kustomize for a given component
$(COMPONENTS):
ifdef RENDER_DIR
	@mkdir -p $(RENDER_DIR)/$(subst clusters/,,$$(dirname $@))
	@kubectl kustomize --enable-helm $@ -o $(RENDER_DIR)/$(subst clusters/,,$@.yaml)
else
	@kubectl kustomize --enable-helm $@
endif

## Remove stale rendered components/crds
cleanup:
	$(eval LIST1 := $(shell mktemp))
	$(eval LIST2 := $(shell mktemp))
	@printf '%s\n' $(sort $(subst .yaml,,$(RENDERED))) >$(LIST1)
	@printf '%s\n' $(sort $(subst cells,rendered,$(COMPONENTS))) >$(LIST2)
	@comm -23 $(LIST1) $(LIST2) | xargs -I % -n1 rm %.yaml
	@rm -rf $(LIST1) $(LIST2)
	$(eval LIST1 := $(shell mktemp))
	$(eval LIST2 := $(shell mktemp))
	@printf '%s\n' $(sort $(subst .yaml,,$(RENDERED_CRDS))) >$(LIST1)
	@printf '%s\n' $(sort $(subst cells,rendered/crds, $(CRDS))) >$(LIST2)
	@comm -23 $(LIST1) $(LIST2) | xargs -I % -n1 rm %.yaml
	@rm -rf $(LIST1) $(LIST2)


.PHONY: $(CLUSTERS) $(COMPONENTS)
